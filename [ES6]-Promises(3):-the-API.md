...
#12. Debugging promises
 비동기 코드를 디버깅하는 가장 주요 문제점은 비동기 함수와 메서드 호출이 포함 되어있다는 것입니다. 비동기 호출은 하나의 *task*에서 생성되어, 새로운 *task*에서 수행되어집니다. 만약 새로운 *task*가 잘못되면, *stack trace*는 이전 *task*에 관한 정보를 포함하지 않은 그 *task*만 적용합니다. 그래서 당신은 비동기 프로그래밍에서 훨씬 적은 디버깅 정보를 사용 해야합니다.<br><br>
